"""``LocalSkillStorage``——本地文件系统实现（M14 skills）。

对齐 deer ``skills/storage/local_skill_storage.py``。布局::

    <root>/public/<name>/SKILL.md
    <root>/custom/<name>/SKILL.md
    <root>/custom/.history/<name>.jsonl

mini 适配：用 ``deerflow.config.paths.resolve_path``（非 deer 的 ``runtime_paths``）。
"""
...
# 安装临时目录清理的尽力超时——卡住的文件系统（如 NFS）不能拖延 install 结果从 finally 传出。
...
class LocalSkillStorage(SkillStorage):
    ...
    async def ainstall_skill_from_archive(self, archive_path: str | Path) -> dict:
        ...
        # 每文件安全审查是 async LLM 调用，必须留事件循环；围绕它的文件系统阶段跑 worker 线程。
