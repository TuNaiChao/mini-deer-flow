"""基于 LangGraph BaseStore 的内存 ThreadMetaStore（MemoryThreadMetaStore）。

``database.backend=memory`` 时使用。委托给 LangGraph Store 的 ``("threads",)``
命名空间——与（未来的）Gateway router 用于线程记录的命名空间相同。
"""
...
class MemoryThreadMetaStore(ThreadMetaStore):
    ...
    def _item_to_dict(item) -> dict[str, Any]:
        """把 Store SearchItem 转成调用方期望的 dict 格式。"""
        val = item.value
        return {
            "thread_id": item.key,
            ...
            # ``coerce_iso`` 修复早期 Gateway 版本用 ``str(time.time())`` 写入的
            # legacy unix 秒值。
