"""skill_manage 工具——创建 / 演化自定义技能。

对齐 deer ``tools/skill_manage_tool.py``。**仅 ``skill_evolution.enabled`` 时绑定**。让 agent 自己
create/patch/edit/delete/write_file/remove_file 自定义技能（``skills/custom/``），全程记
``.history/<name>.jsonl``。per-skill async 锁串行化；写前 LLM 安全扫描（block 抛错、可执行内容须 allow）。

依赖 M14 skills（storage/security_scanner/types，已落地）。写完刷新 lead_agent 的技能提示缓存。
"""
...
# per-skill 锁，WeakValueDictionary 让无引用的锁自动回收。
...
async def _skill_manage_impl(
    ...
    """自定义技能管理（skills/custom/）的实现。``skill_manage_tool`` 是它的 ``@tool`` 包装 + 同步入口。"""
