"""view_image 工具——把图片读成 base64 注入状态（仅 supports_vision 模型）。

对齐 deer ``tools/builtins/view_image_tool.py``。路径白名单仅 ``/mnt/user-data/{workspace,uploads,outputs}``，
20MB 上限，扩展名 + 魔数双校验（防扩展名欺骗）。成功后把 base64 写入 ``viewed_images`` 状态
（经 ``_merge_viewed_images`` reducer 合并），由 ``ViewImageMiddleware``（M16）在 LLM 调用前注入。

仅当模型 ``supports_vision=True`` 时由 ``get_available_tools`` 绑定。
"""
...
def _sanitize_image_error(error: Exception, thread_data) -> str:  # type: ignore[no-untyped-def]
    """错误信息里抹掉宿主真实路径（防泄露目录布局）。"""
...
def view_image_tool(
    ...
    try:
        ...
        return Command(
            update={"messages": [ToolMessage(f"Error: {e}", tool_call_id=tool_call_id)]},
        )

    path = Path(actual_path)

    if not path.exists():
        return Command(update={"messages": [ToolMessage(f"Error: Image file not found: {image_path}", tool_call_id=tool_call_id)]})

    if not path.is_file():
        return Command(update={"messages": [ToolMessage(f"Error: Path is not a file: {image_path}", tool_call_id=tool_call_id)]})
    ...
    try:
        ...
        return Command(update={"messages": [ToolMessage(f"Error reading image metadata: {_sanitize_image_error(e, thread_data)}", tool_call_id=tool_call_id)]})
    if image_size > _MAX_IMAGE_BYTES:
        return Command(update={"messages": [ToolMessage(f"Error: Image file is too large: {image_size} bytes. Maximum supported size is {_MAX_IMAGE_BYTES} bytes", tool_call_id=tool_call_id)]})

    try:
        with open(actual_path, "rb") as f:
            image_data = f.read()
    except Exception as e:  # noqa: BLE001
        return Command(update={"messages": [ToolMessage(f"Error reading image file: {_sanitize_image_error(e, thread_data)}", tool_call_id=tool_call_id)]})

    detected_mime_type = _detect_image_mime(image_data)
    if detected_mime_type is None:
        return Command(update={"messages": [ToolMessage("Error: File contents do not match a supported image format", tool_call_id=tool_call_id)]})
    if detected_mime_type != expected_mime_type:
        return Command(update={"messages": [ToolMessage(f"Error: Image contents are {detected_mime_type}, but file extension indicates {expected_mime_type}", tool_call_id=tool_call_id)]})
    mime_type = detected_mime_type
    image_base64 = base64.b64encode(image_data).decode("utf-8")

    # 写入 viewed_images，由 _merge_viewed_images reducer 合并；ViewImageMiddleware（M16）在 LLM 调用前注入。
