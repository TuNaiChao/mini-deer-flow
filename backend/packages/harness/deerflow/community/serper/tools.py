"""Serper 网页搜索 provider（Google Search API，需 API key）。

对齐 deer ``community/serper/tools.py``：调 Serper（https://serper.dev）的 JSON API，
拿实时 Google 搜索结果。``SERPER_API_KEY`` 来自环境变量或 config.yaml 的 web_search.api_key。
"""
...
def web_search_tool(query: str, max_results: int = 5) -> str:
    ...
    if not api_key:
        ...
        return json.dumps({"error": "SERPER_API_KEY is not configured", "query": query}, ensure_ascii=False)
    ...
    try:
        ...
        logger.error("Serper API returned HTTP %s: %s", e.response.status_code, e.response.text)
        return json.dumps(
            {"error": f"Serper API error: HTTP {e.response.status_code}", "query": query},
            ensure_ascii=False,
        )
    except Exception as e:  # noqa: BLE001
        logger.error("Serper search failed: %s: %s", type(e).__name__, e)
    ...
    normalized_results = [normalize_search_result(title=r.get("title", ""), url=r.get("link", ""), snippet=r.get("snippet", "")) for r in organic[:max_results]]
