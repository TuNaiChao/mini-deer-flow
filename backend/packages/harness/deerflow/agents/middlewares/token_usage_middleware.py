"""``TokenUsageMiddleware``：记录 LLM token 用量 + 给每步贴动作归因（M16）。

每个模型响应后：
  1. 从 ``usage_metadata`` 记 token 用量到日志（含 input/output token 细分）。
  2. 子代理 token：``task`` 工具完成时其用量按 ``tool_call_id`` 缓存（M15 task_tool）。
     检测 ToolMessage → 向前找派发它的 AIMessage → 把子代理 token **合并**进那条
     AIMessage（按消息位置而非 id 合并，处理同一响应多个 task 调用）。
  3. 给最后一条 AIMessage 贴 ``additional_kwargs["token_usage_attribution"]``——结构化
     动作清单（todo / subagent / search / tool 等），前端据此渲染精确的步骤标签而非
     通用「Update to-do list」。这是 write_todos token 归因的**单一真相源**。

本中间件本身不读 config——是否挂载由 ``build_middlewares`` 按 ``token_usage.enabled`` 决定。
"""
...
def _build_todo_actions(previous_todos: list[Todo], next_todos: list[Todo]) -> list[dict[str, Any]]:
    # write_todos token 归因的单一真相源。前端在缺这元数据时回退通用标签。
...
def _describe_tool_call(tool_call: dict[str, Any], todos: list[Todo]) -> list[dict[str, Any]]:
    ...
    if name in {"web_search", "image_search"}:
        ...
        return [{"kind": "search", "tool_name": name, "query": query, "tool_call_id": tool_call_id}]
...
def _build_attribution(message: AIMessage, todos: list[Todo]) -> dict[str, Any]:
    ...
    return {
        # schema 变更尽量 additive，旧前端能忽略未知字段安全回退。
        "version": 1,
...
class TokenUsageMiddleware(AgentMiddleware):
    ...
    def _apply(self, state: AgentState) -> dict | None:
        ...
        # 子代理 token：task 工具完成时按 tool_call_id 缓存（M15）。检测 ToolMessage →
        # 向前找派发 AIMessage → 合并。一条模型响应可派发多个 task，故按位置累计进同一更新。
