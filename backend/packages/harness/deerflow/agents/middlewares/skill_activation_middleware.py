"""显式 slash 技能激活中间件（M14 skills）。

对齐 deer ``agents/middlewares/skill_activation_middleware.py``。用户输 ``/skill-name task``
时，把该 SKILL.md **全文**作为隐藏上下文注入当次模型调用。

关键不变量：
- **幂等**：同一目标 HumanMessage 不重复注入（靠 ``__slash_activation`` ID + additional_kwargs 标志检测）；
- **读盘穿越拒绝**：``_read_skill_content`` 用 ``resolve + relative_to(skills_root)`` 校验；
- **html.escape 防注入**：技能内容 / 用户文本都转义后再嵌进提示；
- **未启用失败**：技能未安装 / 未启用 / 不在白名单 → 返回友好的 AIMessage 失败提示；
- **async 卸载**：``awrap_model_call`` 用 ``asyncio.to_thread`` 跑读盘。
"""
...
class SkillActivationMiddleware(AgentMiddleware):
    """用户显式输 ``/skill-name`` 时注入完整 SKILL.md 内容。"""
    ...
    def _read_skill_content(skill_file: Path, skills_root: Path) -> str:
        """读 SKILL.md；必须 resolve 后仍在 skills_root 内（防穿越），否则抛 ValueError。"""
    ...
    def _build_activation_reminder(activation: _Activation) -> str:
        """构造激活提醒串。所有动态内容 html.escape 防注入。"""
...
{escaped_skill_content}
    ...
    def _make_activation_message(target: HumanMessage, activation_content: str) -> HumanMessage:
        """构造隐藏的激活 HumanMessage（派生 ID 关联目标，幂等检测用）。"""
    ...
    async def awrap_model_call(
        ...
        # _prepare_model_request 读盘（同步 IO），用 to_thread 卸载避免阻塞事件循环。
