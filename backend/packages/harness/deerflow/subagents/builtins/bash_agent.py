...
只挂沙箱 5 工具（bash/ls/read_file/write_file/str_replace），``max_turns=60``。
当 host bash 未被放行（``is_host_bash_allowed()=False``）时，registry 的
``get_available_subagent_names`` 会把 ``bash`` 从可见列表里隐藏（见 registry.py）。
"""
