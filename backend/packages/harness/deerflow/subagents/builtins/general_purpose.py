"""通用子代理配置（``general-purpose``）。

继承父 agent 全部工具（除 task/ask_clarification/present_files，防嵌套与澄清），
``max_turns=150`` 适合多步探索+动作的深任务（从 100 上调，避免开箱即撞
``GraphRecursionError``）。
"""
...
""",
    ...
    disallowed_tools=["task", "ask_clarification", "present_files"],  # 防嵌套与澄清
    model="inherit",
    max_turns=150,
)
