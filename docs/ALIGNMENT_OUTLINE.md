...
> **版本：v1.1（已修订）** — 在 v1 基础上补齐 models / build / 可选模块、修正执行顺序循环依赖、补全工程化与样例。修订记录见文末「修订日志」。
>
> 本文档是 **master outline**：综合 agent / tools / skills / middleware / memory / models / runtime / persistence / 工程化 全部模块的对齐分析，给出可被后续 AI 直接执行的文件级修改计划。
>
> **目标**：让 mini-deer-flow 在行为上对齐 deer-flow，且**每个模块可用、可靠**（有默认值、IO 卸载、错误兜底、可测试）。
>
> **执行约定**：
...
models(factory 升级) ◄── config/model_config ──┐                      │
   ...
   agents/memory ◄── models/create_chat_model ──────────────────────────┤
   skills ──────────────────────────────────────────────────────────────┤
                                                                         ▼
   tools(builtins/mcp_metadata/sync/skill_manage) ─┐
   agents/middlewares(新增+重做) ──────────────────┤
...
[可选模块，本期不做/留接口]：community(web 搜索) / mcp / uploads / agents_config(custom agent)
```
...
| **2 沙箱/子代理/追踪** | sandbox（M10）、subagents（M11）、tracing（M12） | 执行隔离 + 委派 + 链路追踪 | ✅ 单测 |
| **3 记忆** | agents/memory（M13） + memory_middleware/dynamic_context 重做 | 真记忆写入/注入 | ✅ `test_memory` |
| **4 技能** | skills（M14） + skill_activation + prompt 集成 | /skill 激活 + allowed-tools | ✅ `test_skills` |
| **5 工具** | tools（M15） | 完整工具集 | ✅ `test_tools` |
| **6 中间件** | middlewares（M16）新增 16 + 重做 8 | 23 步中间件链 | ✅ `test_middlewares` |
| **7 Agent 装配** | agents（M17）factory/features/thread_state/lead_agent/prompt/build_middlewares | SDK + config 双入口 | ✅ `test_agent` |
...
| **可选（按需，不阻塞主线）** | community（M-opt-community）、mcp（M-opt-mcp）、uploads（M-opt-uploads）、agents_config（M-opt-agents_config）、DeerFlowClient | 联网搜索 / MCP / 文件上传 / 自定义 agent / 嵌入式客户端 | 视需求 |
...
12. **M10** sandbox ⬜ —— 依赖 config/sandbox_config + user_context + paths；提供 bash 等工具与 SandboxMiddleware。
13. **M11** subagents ⬜（骨架）—— registry/status_contract/executor；真实 agent 构造依赖 M17，但骨架可先建。
14. **M12** tracing ⬜ —— 依赖 user_context；落地后 M-models 的 `attach_tracing=True` 路径自动生效。

**Phase 3 — 记忆**

15. **M13** memory ⬜ —— 依赖 M-models（`create_chat_model`）+ config/memory_config + user_context + paths；重做 MemoryMiddleware + DynamicContextMiddleware。

**Phase 4 — 技能**

16. **M14** skills ⬜ —— 依赖 config/skills_config + extensions_config + reflection + utils/messages；含 SkillActivationMiddleware。

**Phase 5 — 工具**

17. **M15** tools ⬜ —— 依赖 sandbox/subagents/models/config；补全 builtins + MCP 标记 + sync + 去重。

**Phase 6 — 中间件**

18. **M16** middlewares ⬜ —— 依赖几乎所有业务模块（sandbox/subagents/skills/memory/tracing）；新增 16 + 重做 8，重写 `build_middlewares` 为 23 步。

**Phase 7 — Agent 装配**

19. **M17** agents ⬜ —— 依赖 Phase 2–6 + tools + checkpointer；factory/features/thread_state/lead_agent/prompt。完成后 M11 的真实 agent 构造才可用。

**Phase 8 — 运行管理 + 集成**

20. **M18** runs（manager/worker/store.memory）⬜ —— 依赖 M17 agent factory + 全部 runtime 基础；run 生命周期 + lifespan 装配。
21. **M19** runtime/store ⬜ —— LangGraph BaseStore 工厂（memory 版即可）。
22. **集成装配** ⬜ —— lifespan（init_engine + make_checkpointer + make_stream_bridge + make_thread_store + RunManager + reconcile + shutdown）+ 对齐 langgraph.json + config.example.yaml。端到端冒烟。

**可选模块（任何时候按需，不阻塞主线）**

- **M-opt-community**（web 搜索，建议优先 1 个让 agent 可联网）/ **M-opt-mcp** / **M-opt-uploads** / **M-opt-agents_config** / DeerFlowClient。

> 并行提示：Phase 1 内 M9/M8 与 M5–M7 无强依赖，可并行；Phase 2 三个模块相互独立可并行。但**单线序列**最稳妥，避免合并冲突与依赖错配。
...
### M10. sandbox（沙箱）— Phase 2

- **现状**：无（config 引用 `deerflow.sandbox.local:LocalSandboxProvider` 但模块不存在）。
- **目标**：本地代码执行隔离 + bash/ls/read_file/write_file/str_replace 工具。
- **文件清单**：
  - **新建**：`sandbox/__init__.py`、`sandbox/sandbox.py`（抽象 `Sandbox`）、`sandbox/sandbox_provider.py`（`SandboxProvider` + 单例 `get_sandbox_provider`）、`sandbox/tools.py`（5 工具 + `replace_virtual_path*`，工具经 `Runtime` 注入读 state 的 sandbox）、`sandbox/middleware.py`（`SandboxMiddleware`）、`sandbox/security.py`（`is_host_bash_allowed`）、`sandbox/local/__init__.py`、`sandbox/local/local_sandbox.py`（`LocalSandboxProvider` + `LocalSandbox`，每线程 path_mappings + LRU + `is_local_sandbox`）
- **关键要点**：虚拟路径 `/mnt/user-data/{workspace,uploads,outputs}`、`/mnt/skills` → 物理目录翻译；每线程 `local:{thread_id}`；`SandboxMiddleware(lazy_init)` 写 `sandbox_id` 到 state。
- **依赖**：config/sandbox_config、runtime/user_context、config/paths。
- **可靠性**：路径翻译作 defense-in-depth；LRU 上限防泄漏。
- **测试**：`backend/test/test_sandbox.py` —— acquire/release、虚拟路径翻译、bash 执行、5 工具、host-bash 过滤、LRU、Runtime 注入读 sandbox。
- **学习文档**：`docs/sandbox.md` —— 虚拟路径系统、provider 模式、为什么 local 模式要 path translation、工具如何从 state 取 sandbox。
- **裁剪**：Docker/AIO provisioner **不做**。

---

### M11. subagents（子代理）— Phase 2

- **现状**：无。
- **目标**：`task` 工具委派 + 后台执行 + 限流。
- **文件清单**：**新建** `subagents/__init__.py`、`subagents/registry.py`（`get_subagent_config`/`get_available_subagent_names`）、`subagents/executor.py`（`SubagentExecutor` 双线程池 + 超时 + SSE 事件）、`subagents/status_contract.py`（`extract_subagent_status`/`make_subagent_additional_kwargs`）、`subagents/builtins/__init__.py`（general-purpose + bash）。
- **关键要点**：`MAX_CONCURRENT_SUBAGENTS=3`；`_scheduler_pool` + `_execution_pool`（各 3 worker）；子代理图 `checkpointer=False`（一次性）；事件 `task_started/running/completed/failed/timed_out`。
- **依赖**：agents（创建子代理图，Phase 7 —— **注意**：executor 在 Phase 8 worker 中被 task_tool 调用；Phase 2 只建 registry/status_contract/executor 骨架，实际 agent 构造依赖 Phase 7）。
- **可靠性**：超时 1800s；线程池隔离；状态契约统一。
- **测试**：`backend/test/test_subagents.py` —— 注册、status_contract、executor 成功/超时/失败（用 mock agent）、限流截断。
- **学习文档**：`docs/subagents.md` —— 双线程池设计、为何 checkpointer=False、并发上限如何由中间件 + 执行器共同保证。
- **裁剪**：可先做 general-purpose，bash 子代理依赖 sandbox。
...
- **依赖**：**models（M-models，`create_chat_model`）**、config/memory_config、runtime/user_context、config/paths。`AGENT_NAME_PATTERN` 在 `agents_config` 未落地前于 storage.py 局部定义兜底。
...
- **文件清单**：
  ...
  - **修改**：`config/extensions_config.py`（补 `is_skill_enabled`）、`agents/lead_agent/prompt.py`（移植 `get_skills_prompt_section` + 后台刷新缓存）、`agents/lead_agent/agent.py`（`filter_tools_by_skill_allowed_tools`）
  - **新建目录**：`mini-deer-flow/skills/public/<示例>/SKILL.md`
- **关键要点**：slash 严格语法 + 保留字过滤；激活读盘必 `relative_to(skills_root)` 防穿越 + html.escape 防注入；安装 zip 炸弹防御 + symlink 跳过 + 预占目标原子搬入；enabled 状态每次重读 extensions_config；`load_skills` 文件 IO `to_thread`。
...
### M15. tools（工具模块）— Phase 5

- **现状**：仅 `present_file_tool` + `ask_clarification_tool`；`get_available_tools` 精简。
- **目标**：对齐 deer 的工具组装（内置补全 + MCP 标记 + sync 包装 + 去重 + 条件加载）。
- **文件清单**：
  - **新建**：`tools/builtins/view_image_tool.py`、`tools/builtins/task_tool.py`、`tools/builtins/setup_agent_tool.py`（**可选**，依赖 M-opt-agents_config）、`tools/builtins/update_agent_tool.py`（**可选**）、`tools/builtins/tool_search.py`（**可选**，依赖 M-opt-mcp）、`tools/mcp_metadata.py`（`tag_mcp_tool`）、`tools/sync.py`（`make_sync_tool_wrapper`）、`tools/skill_manage_tool.py`（**可选**）
  - **修改**：`tools/builtins/__init__.py`（导出补全）、`tools/tools.py`（`get_available_tools` 重写：host-bash 过滤 + name-mismatch 告警 + sync 包装 + MCP（可选 try/except）+ view_image 条件 + 去重）、`tools/types.py`（对齐）
...
- **文件清单**：
  ...
  - **新建（可选档，按依赖）**：`uploads_middleware.py`（**依赖 M-opt-uploads，本期不做**）、`sandbox_audit_middleware.py`（依赖 sandbox）、`summarization_middleware.py`、`token_usage_middleware.py`、`view_image_middleware.py`、`deferred_tool_filter_middleware.py`（依赖 tool_search，**可选**）、`subagent_limit_middleware.py`（依赖 subagents）、`safety_finish_reason_middleware.py` + `safety_termination_detectors.py`、`skill_activation_middleware.py`（已在 M14）
  - **重做现有**：`memory_middleware.py`（M13）、`dynamic_context_middleware.py`（M13）、`tool_error_handling_middleware.py`（补 task-status stamping）、`clarification_middleware.py`（补 options/context）、`title_middleware.py`（config 驱动 + 结构化内容归一）、`llm_error_handling_middleware.py`（对齐）、`loop_detection_middleware.py`（`from_config`）
...
- **依赖**：**models（M-models）**、Phase 2–6 全部 + tools + runtime（checkpointer）。
- **prompt 条件段（S11 明确）**：`SYSTEM_PROMPT_TEMPLATE` 的占位按 feature 填充——`{skills_section}`(M14，enabled skills 非空)、`{deferred_tools_section}`(tool_search.enabled)、`{subagent_section}`/`{subagent_reminder}`/`{subagent_thinking}`(subagent_enabled)、`{soul}`+`{self_update_section}`(agent_name，依赖 M-opt-agents_config，未做则空)、`{acp_section}`(ACP，不做则空)。未启用段返回 `""`。
...
> 以下模块对齐 deer，但**本期不实现**，仅在相关模块留 `try/except ImportError` 接口与 install hint。需求出现时按此清单补。

| 模块 | 路径 | 作用 | 依赖它的模块 | 本期处理 |
|------|------|------|--------------|----------|
| **M-opt-community** | `community/` | web 搜索/抓取（tavily/ddg/jina/firecrawl/...） | tools（web_search/web_fetch）、deep-research | 留接口；建议优先移植 1 个（tavily 或 ddg_search）让 agent 可联网 |
| **M-opt-mcp** | `mcp/` | MCP 服务器工具（cache/client） | tools MCP 加载、tool_search、deferred_tool_filter | 留接口；tools.py 内 try/except 软加载 |
| **M-opt-uploads** | `uploads/` | 文件上传 + markitdown 转换 | UploadsMiddleware（M16 第 3 步） | 不做；UploadsMiddleware 同步不做 |
| **M-opt-agents_config** | `config/agents_config.py` | 自定义 agent SOUL.md/config | setup_agent/update_agent 工具、memory `AGENT_NAME_PATTERN` | 不做；`AGENT_NAME_PATTERN` 在 memory 局部定义兜底，setup/update_agent 工具不做 |
...
18. DeferredToolFilterMiddleware        [tool_search, 可选]
19. SubagentLimitMiddleware             [subagent]
...
补 `database`(backend: memory)、`run_events`、`stream_bridge`、`skills`(path/container_path)、`skill_evolution`(enabled: false)、`tool_output`、`token_usage`、`safety_finish_reason` 等段，全部带安全默认值；改 schema 时同步 bump `config_version`。
...
| M-opt-community/mcp/uploads/agents_config | （留接口） | — | （需求时补） |
...
4. **裁剪项明确不做**：见「可选模块」表；标注「可选/本期不做」的留接口不实现。
